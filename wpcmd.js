...
wpcmd.generalKeyUp = function (event) {
    ...
    if (target.tagName.toLocaleLowerCase() !== "input" || !wpcmd.config.dontCaptureOnInputElements) {
        if (target.tagName.toLocaleLowerCase() !== "textarea") {
            ...
            var keyCode = event.keyCode || window.event.which;

            if (wpcmd.pressedKeys.length < wpcmd.config.maxCommandLength) {
                wpcmd.pressedKeys += keyCode;
                wpcmd.typedCommand += String.fromCharCode(keyCode).toLowerCase();
            } else
                wpcmd.pressedKeys = keyCode;
